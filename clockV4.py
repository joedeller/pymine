#! /usr/bin/python
# Joe Deller 2014
# A "2D"  version of our digital clock

# Level : Intermediate
# Uses  : Libraries, variables, dictionary, operators, loops, logic

# This time we only draw the numbers at the same depth as the 
# black background.  Instead of drawing AIR, we draw black wool
# when the character has a space.  Now we can see around the back of our clock
...
def CheckBlockIsHit():
    ...
    if (blockHits):
        for blockHit in blockHits:
            ...
            if (blockHit.pos.x == stoneX and blockHit.pos.y == stoneY and blockHit.pos.z == stoneZ):
...
# clock color
mc.setBlock(stoneX, stoneY, stoneZ, block.STONE.id)
...
hoursX = x
minutesX = x + (letterWidth * 3)
secondsX = x + (letterWidth * 6)

# mc.player.setTilePos(x+24,y,playerPos.z)
...
while True:
    ...
    if (CheckBlockIsHit()):
        ...
        # print "hour =" +hour +" min = "+minute +" seconds "+seconds
    ...
    if (hour != last_hour):
        DrawMessage(hoursX, hour)
    # The minutes are to the right 18 spaces (-18), 2 characters for the hours plus one for the ":"
    # we should really calculate the number, 3 * letter width, but for now
    # addition is faster!
    if (minute != last_minute):
        DrawMessage(minutesX, minute)
    # The seconds are to the right 36 spaces, hh:mm:ss

    tens = seconds[:1]
    if (tens == last_seconds[:1]):
        units = seconds[1:]
        DrawMessage(secondsX + 6, units)
    else:
        DrawMessage(secondsX, seconds)
    # reset the location to the left of the clock
